        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="border border-light-border dark:border-dark-border rounded-lg px-3 py-2 bg-light-background dark:bg-dark-background text-light-text dark:text-dark-text focus:ring-primary-500 dark:focus:ring-primary-400"
      ...
      <div className="flex-1 overflow-y-auto bg-light-card dark:bg-dark-card border-t border-light-border dark:border-dark-border">
        ...
        <ul className="space-y-4 p-4">
          {sortedPrograms.map((program) => {
            ...
            return (
                <li key={program.id} className="border border-light-border dark:border-dark-border bg-light-card dark:bg-dark-card rounded-lg shadow dark:shadow-dark-border/30 p-4">
                <article>
                  <div className="flex items-start justify-between">
                    <div>
                      {/* ✅ SEO-friendly heading */}
                        <h2 className="font-semibold text-lg mb-2 text-light-text dark:text-dark-text">{program.title}</h2>
                        <div className="flex items-center gap-4 text-sm text-light-muted dark:text-dark-muted mb-2">
                        <div className="flex items-center gap-1">
                            <Building className="w-4 h-4 text-primary-500 dark:text-primary-400" aria-hidden="true" />
                          <span>{program.organization}</span>
                        </div>
                        <div className="flex items-center gap-1">
                            <Calendar className="w-4 h-4 text-primary-500 dark:text-primary-400" aria-hidden="true" />
                          <span>Due: {formattedDate}</span>
                        </div>
                        {program.startDate && (
                          <div className="flex items-center gap-1">
                              <Clock className="w-4 h-4 text-primary-500 dark:text-primary-400" aria-hidden="true" />
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleDeleteProgram(program.id)}
                          className="text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 p-2 transition-colors"
                  ...
                  <div className="mt-4">
                      <h3 className="font-medium mb-2 text-light-text dark:text-dark-text">Requirements</h3>
                    {Array.isArray(program.requirements) ? (
                        <ul className="list-disc list-inside text-sm text-light-muted dark:text-dark-muted">
                    ...
                    {program.applicationUrl && (
                      <div className="mt-4">
                        <a 
                          ...
                          className="inline-block px-4 py-2 bg-primary-600 dark:bg-primary-700 text-white rounded-lg hover:bg-primary-700 dark:hover:bg-primary-600 transition-colors"
                        >
                          Apply Now
                        </a>
                      </div>
