...
export async function GET(req: NextRequest) {
  try {
    ...
    studentStageAverages.forEach(row => {
      ...
      let finalStage = 'Early';
      if (avgStageNum >= 2.5) finalStage = 'Late';
      else if (avgStageNum >= 1.5) finalStage = 'Mid';
      else finalStage = 'Early';
      
      studentStageMap.set(studentId, finalStage);
    });
    ...
    const studentsWithProgress = studentRows.map(student => {
      ...
      // Get assessment stage for this student
      const studentAssessmentStage = studentStageMap.get(student.user_id) || 'Early';
    ...
    studentStageMap.forEach((stage) => {
      ...
      else if (stage === 'Mid') stageDistribution.Mid++;
      else if (stage === 'Late') stageDistribution.Late++;
    });
    
    // Convert to percentages (only count students with assessment data)
    const studentsWithAssessments = studentStageMap.size;
    Object.keys(stageDistribution).forEach((key: string) => {
      const stageKey = key as keyof typeof stageDistribution;
      stageDistribution[stageKey] = studentsWithAssessments > 0 ? Math.round((stageDistribution[stageKey] / studentsWithAssessments) * 100) : 0;
    });
